"""Grid layout for UluxDisplay displays."""
...
class GridLayout(Layout):
    ...
    def __init__(self, rows: int = 2, cols: int = 2, padding: int = 8, gap: int = 8) -> None:
    ...
    def _calculate_slots(self) -> None:
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                self.slots.append(Slot(index=slot_index, rect=(x1, y1, x1 + cell_width, y1 + cell_height)))
                slot_index += 1
