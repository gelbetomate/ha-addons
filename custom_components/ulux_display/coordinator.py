...
class UluxDisplayCoordinator(DataUpdateCoordinator):
    """Coordinator for UluxDisplay display updates."""
    ...
    def __init__(
        ...
        Args:
            ...
            device: UluxDisplay device client
        ...
        _LOGGER.debug(
            "Initialized UluxDisplay coordinator for %s with refresh interval %ds",
            device.host,
            interval,
        )
    ...
    def _get_store(self) -> UluxDisplayStore | None:
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._device_offline:
                ...
                _LOGGER.info(
                    "UluxDisplay device %s is back online after %d failed attempts",
                    self.device.host,
    ...
    def _log_offline_status(self, message: str) -> None:
        ...
        if self._consecutive_failures == 1:
            ...
            _LOGGER.warning(
                "UluxDisplay device %s is offline: %s. Will retry with exponential backoff.",
                self.device.host,
                message,
            )
            ...
            _LOGGER.warning(
                "UluxDisplay device %s still offline after %d attempts (retry interval: %ds)",
                self.device.host,
                self._consecutive_failures,
                interval,
            )
            ...
            _LOGGER.debug(
                "UluxDisplay device %s offline (attempt %d): %s",
                self.device.host,
                self._consecutive_failures,
                message,
            )
    ...
    def _log_connection_error(self, err: Exception) -> None:
        ...
        if self._consecutive_failures == 1:
            ...
            _LOGGER.warning(
                "UluxDisplay device %s connection failed: %s. Will retry with exponential backoff.",
                self.device.host,
                err,
            )
        elif self._consecutive_failures % BACKOFF_LOG_INTERVAL == 0:
            ...
            _LOGGER.warning(
                "UluxDisplay device %s still failing after %d attempts: %s (retry interval: %ds)",
                self.device.host,
                self._consecutive_failures,
                err,
                interval,
            ...
            # Subsequent failures - log at debug level only
            _LOGGER.debug(
                "UluxDisplay update failed (attempt %d): %s",
                self._consecutive_failures,
                err,
            )
    ...
    def device_name(self) -> str:
        ...
        return f"UluxDisplay {self.device.host}"
    ...
    def get_store(self) -> UluxDisplayStore | None:
