"""Entity widget for UluxDisplay displays."""
...
class EntityWidget(Widget):
    ...
    SCHEMA: ClassVar[dict[str, Any]] = {
        ...
        "options": [
            ...
            {"key": "precision", "type": "number", "label": "Decimal Places", "min": 0, "max": 5},
        ],
    }
    ...
    def render(self, ctx: RenderContext, state: WidgetState) -> Component:
        ...
        if icon:
            content = IconValue(icon=icon, value=value_text, label=label or "", color=color, value_color=THEME_TEXT_PRIMARY, label_color=THEME_TEXT_SECONDARY)
        else:
            content = CenteredValue(value=value_text, label=label, value_color=THEME_TEXT_PRIMARY, label_color=THEME_TEXT_SECONDARY)
