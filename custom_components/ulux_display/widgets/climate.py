"""Climate widget for UluxDisplay displays."""
...
class ClimateDisplay(Component):
        ...
        if is_small:
            ...
            if self.show_humidity and self.humidity is not None:
                try:
                    ...
                    pass
            if row1_parts:
                children.append(Row(children=row1_parts, gap=8, align="center", justify="center"))
            if self.show_mode:
                display_text = self.hvac_action or self.hvac_mode
                if display_text:
                    children.append(Text(display_text.replace("_", " ").title(), font="tiny", color=color))
        else:
            bottom_parts: list[Component] = []
            if self.show_target and self.target_temp is not None:
                target_str = _format_temp(self.target_temp)
                bottom_parts.append(Row(children=[Icon("target", size=10, color=THEME_TEXT_SECONDARY), Text(target_str, font="tiny", color=THEME_TEXT_SECONDARY)], gap=2, align="center"))
            if self.show_humidity and self.humidity is not None:
                try:
                    humidity_val = int(float(self.humidity))
                    bottom_parts.append(Row(children=[Icon("water-percent", size=10, color=COLOR_CYAN), Text(f"{humidity_val}%", font="tiny", color=COLOR_CYAN)], gap=2, align="center"))
                except (ValueError, TypeError):
                    pass
            if self.show_mode:
                display_text = self.hvac_action or self.hvac_mode
                if display_text:
                    bottom_parts.append(Text(display_text.replace("_", " ").title(), font="tiny", color=color))
            if bottom_parts:
                children.append(Row(children=bottom_parts, gap=12, align="center", justify="center"))

        return Column(children=children, gap=int(height * 0.04), padding=padding, align="center", justify="center")
    ...
    def _build_compact(self, ctx: RenderContext, width: int, height: int) -> Component:
        ...
        if has_room_for_details:
            ...
            top_row = Row(children=[Icon(icon_name, size=icon_size, color=color), Text(current_str, font="medium", color=THEME_TEXT_PRIMARY)], gap=4, align="center", justify="center")
            detail_parts: list[Component] = []
            if self.show_target and self.target_temp is not None:
                target_str = _format_temp(self.target_temp)
                detail_parts.append(Row(children=[Icon("target", size=8, color=THEME_TEXT_SECONDARY), Text(target_str, font="tiny", color=THEME_TEXT_SECONDARY)], gap=2, align="center"))
            if self.show_humidity and self.humidity is not None:
                try:
                    humidity_val = int(float(self.humidity))
                    detail_parts.append(Row(children=[Icon("water-percent", size=8, color=COLOR_CYAN), Text(f"{humidity_val}%", font="tiny", color=COLOR_CYAN)], gap=2, align="center"))
                except (ValueError, TypeError):
                    pass
            children: list[Component] = [top_row]
            if detail_parts:
                children.append(Row(children=detail_parts, gap=6, align="center", justify="center"))
            return Column(children=children, gap=2, padding=padding, align="center", justify="center")

        icon_size = max(14, min(24, int(height * 0.35)))
        return Row(children=[Icon(icon_name, size=icon_size, color=color), Text(current_str, font="small", color=THEME_TEXT_PRIMARY)], gap=padding, align="center", justify="center", padding=padding)
