...
def header_height_for(
    ...
    *,
    ...
    """Header height for the given mode."""
    if mode == "stacked":
...
def render_label_value_header(
    ...
    *,
    ...
    if mode == "stacked":
        Column(
            children=[
                Text(text=label.upper(), font="small", color=THEME_TEXT_SECONDARY, align="center", truncate=True),
                Text(text=value, font="regular", color=value_color, align="center", auto_fit=True),
            ],
            gap=2, padding=2, align="stretch", justify="center",
        ).render(ctx, x, y, width, header_height)
    elif mode == "inline":
        Row(
            children=[
                Text(text=label.upper(), font="small", color=THEME_TEXT_SECONDARY, align="start", truncate=True),
                Spacer(),
                Text(text=value, font="regular", color=value_color, align="end", auto_fit=True),
            ],
            gap=4, padding=padding, align="center", justify="start",
        ).render(ctx, x, y, width, header_height)
    elif mode == "value_only":
        Row(
            children=[Text(text=value, font="regular", color=value_color, align="center", auto_fit=True)],
            padding=padding, align="center", justify="center",
        ).render(ctx, x, y, width, header_height)
    elif mode == "label_only":
        Row(
            children=[Text(text=label.upper(), font="small", color=THEME_TEXT_SECONDARY, align="center", truncate=True)],
            padding=padding, align="center", justify="center",
        ).render(ctx, x, y, width, header_height)
