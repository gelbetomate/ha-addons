"""Chart widget for UluxDisplay displays."""
...
class ChartDisplay(Component):
    ...
    def render(self, ctx: RenderContext, x: int, y: int, width: int, height: int) -> None:
        ...
        render_label_value_header(ctx, x, y, width, header_height, mode=mode, label=self.label, value=value_str, value_color=self.color, padding=padding)
        if len(self.data) >= 2:
            if is_binary:
                ctx.draw_timeline_bar(chart_rect, self.data, on_color=self.color)
            else:
                ctx.draw_sparkline(chart_rect, self.data, color=self.color, fill=self.fill, gradient=self.gradient)
                if show_range:
                    min_val = min(self.data)
                    max_val = max(self.data)
                    range_y = chart_bottom + int(height * 0.08)
                    ctx.draw_text(f"{min_val:.1f}", (x + padding, range_y), font=font_label, color=THEME_TEXT_SECONDARY, anchor="lm")
                    ctx.draw_text(f"{max_val:.1f}", (x + width - padding, range_y), font=font_label, color=THEME_TEXT_SECONDARY, anchor="rm")
        else:
...
class ChartWidget(Widget):
    ...
    SCHEMA: ClassVar[dict[str, Any]] = {
        ...
        "entity_domains": None,
        "options": [
            {"key": "period", "type": "select", "label": "Period", "options": ["5 min", "15 min", "1 hour", "6 hours", "24 hours"], "default": "24 hours"},
            {"key": "show_value", "type": "boolean", "label": "Show Current Value", "default": True},
            {"key": "show_range", "type": "boolean", "label": "Show Min/Max Range", "default": True},
            {"key": "fill", "type": "boolean", "label": "Fill Area", "default": False},
            {"key": "color_gradient", "type": "boolean", "label": "Value Gradient", "default": False},
        ],
    }
